...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query(value = "SELECT COUNT(t) FROM Task t WHERE t.project.projectCode = ?1 AND t.taskStatus <> 'Completed'")
    ...
    List<Task> findAllByTaskStatus(Status status, User assignedEmployee);
